...
class ModCog(commands.Cog):
    ...
    async def ban(self, ctx, member: Member, *, reason=None):
        try:
            ...
            await ctx.send(embed=Embed(title="Ban", description=f"Hey {ctx.author.mention} you have succsesfully banned {member}", color=self.bot.color))
        except:
    ...
    async def poll(self, ctx, *, desc):
        message = await ctx.send(embed=Embed(title="Poll", description=f"{ctx.author.mention} Started a poll!", color=self.bot.color).add_field(name="Please React", value=f"**{desc}**", inline=False).set_footer(text="👍 for Yes, 👎 for No."))
    ...
    async def lock(self, ctx):
        for role in ctx.guild.roles:
            if not role.permissions.manage_channels: #closing channel for everyone that doesnt have manage_channels perm
    ...
    async def slowmode(self, ctx, seconds: int):
        ...
        await ctx.send(embed=Embed(title="Slowmode", description=f"I have set the slowmode for <#{ctx.message.channel.id}> to `{seconds}`", color=self.bot.color))
    ...
    async def snipe(self, ctx):
        try:
            ...
            await ctx.send(embed=Embed(title="Snipe", color=self.bot.color, timestamp=datetime.utcnow()
