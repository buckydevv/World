...
sp = spotipy.Spotify(auth_manager=SpotifyClientCredentials(client_id=os.environ["SP_ID"], client_secret=os.environ["SP_SECRET"]))
    ...
    async def spotify(self, ctx, user: Optional[discord.Member], option: Optional[str], *, song: Optional[str]):
        if option == "--artist":
        	...
        	for track in results:
        		...
        		embed = discord.Embed(title=artist['name'], color=0x2F3136)
        ...
        if option == "--song":
        	...
        	for track in results:
        		...
        		try:
        			...
        			return await ctx.send(f"Sorry {ctx.author.mention} but that artist does not exist!")
        		embed = discord.Embed(title=song['name'], color=0x2F3136)
